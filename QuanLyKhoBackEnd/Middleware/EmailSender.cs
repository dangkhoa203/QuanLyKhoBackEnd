...
namespace QuanLyKhoBackEnd.Middleware {
    ...
    public class EmailSender {
        private EmailSenderConfig Configuration { get; }
        ...
        public async Task<bool> SendEmail(string toEmail, string subject, string content, string confirmLink, string linkAction) {
            try {
                string body=GetBody(content, confirmLink,linkAction);
                using (SmtpClient client = new SmtpClient("smtp.gmail.com")) {
                    client.Port = 587;
                    client.Credentials = new NetworkCredential(Configuration.Email, Configuration.Password);
                    client.EnableSsl = true;
                    using (var message = new MailMessage(Configuration.Email, toEmail)) {
        ...
        private string GetBody(string content, string confirmLink, string linkAction) {
            string body = $@"
                              <!doctype html>
